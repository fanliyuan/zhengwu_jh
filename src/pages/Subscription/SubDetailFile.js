...
import { Form, Input, DatePicker, Button, Table, message, Row, Col } from 'antd';
...
export default class SubDetailFile extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'subDetailFile/getDataList',
      payload: {
        id: match.params.id,
        query: {
          pageNum: 1,
  ...
  render() {
    ...
    const {
      subDetailFile: { dataList, fileInfo },
      loadingTable,
    } = this.props;
    const { selectedIds } = this.state;
    const {
