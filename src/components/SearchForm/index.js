...
export default class SearchForm extends Component {
  ...
  handleReset = () => {
    const {
      ...
      formOptions: { searchHandler = () => {}, resetHandler = () => {} },
    } = this.props;
    resetFields();
    const data = getFieldsValue();
    // console.log(data)
  ...
  render() {
    const {
      ...
      formOptions: { formData = [], hasReset = true } = {},
    } = this.props;
    const FormItems = [];
        ...
        case 'Cascader':
          FormItems.push(
            <FormItem className="w150 fl mr16" {...item.itemOptions} key={item.name + item.type}>
              {getFieldDecorator(item.name)(<Cascader {...item.typeOptions} />)}
            </FormItem>
          );
          break;
        case 'RangePicker':
          FormItems.push(
            <FormItem className="w220 fl mr16" {...item.itemOptions} key={item.name + item.type}>
              {getFieldDecorator(item.name)(<DatePicker.RangePicker />)}
            </FormItem>
          );
          break;
        case 'Checkbox':
    ...
    FormItems.push(
      <FormItem className="w82 fl mr16" key="searchbutton">
    ...
    if (hasReset) {
      FormItems.push(
        <FormItem className="w64 fl" key="resetbutton">
